...
def transactions():
    ...
    month_start = date(date.today().year, date.today().month,
                       1).strftime("%Y-%m-%d")
    ...
    curr_response: TC.TransactionResponse = TC.get_transactions_by_date_range(
        start=month_start, end=today)
    pend_response: TC.TransactionResponse = TC.get_transactions_by_date_range(
        start=tomorrow)
    past_response: TC.TransactionResponse = TC.get_transactions_by_date_range(
    ...
    if form.validate_on_submit():
        if form.transaction_type.data in ('trfr', 'ccp', 'finpay'):
            ...
            if form.transaction_type.data == 'trfr':
                new_category = [
                    category for category in form.category.choices if category[1] == 'Transfer'][0]
            elif form.transaction_type.data == 'ccp':
                new_category = [
                    category for category in form.category.choices if category[1] == 'Card Payment'][0]
            elif form.transaction_type.data == 'finpay':
                new_category = [
                    category for category in form.category.choices if category[1] == 'Finance Payment'][0]
            else:
                new_category = [1]
...
def update_transaction(transactionid: int):
    response: TC.TransactionResponse = TC.get_transaction_by_id(
    ...
    if form.validate_on_submit():
        update_response: TC.TransactionResponse = TC.update_transaction(
            transaction_data=form.to_json())
